...
class DebateKellyStrategy:
    ...
    async def on_event(self, event: Event, ctx: BacktestContext) -> None:
        ...
        if isinstance(event, NewsEvent) and event.ticker:
            ...
            bull_score = self.news_weight * news_conf + self.momentum_weight * max(
                0, momentum_signal * 10
            )
            # Bear argues: combine negative news + negative momentum
            bear_score = self.news_weight * (
                1 - news_conf
...
def print_report(
    ...
    print(f'\n{sep}')
    print(
        f'  BEST:  {best.name} ({best.return_pct:+.2f}% return, '
        f'Sharpe={best.sharpe_ratio:.3f})'
    )
    print(
        f'  WORST: {worst.name} ({worst.return_pct:+.2f}% return, '
...
async def main() -> None:
    ...
    for name, strat in strategies:
        ...
        logger.info(
            f'  {name}: return={result.return_pct:+.2f}%, trades={result.total_trades}'
        )
