...
class TestDataManager:
    def test_empty_market_data(self, market_data: DataManager):
    ...
    def test_custom_spread(self, test_ticker: PolyMarketTicker):
        ...
        mdm = DataManager(spread=Decimal('0.02'))
    ...
    def test_custom_synthetic_size(self, test_ticker: PolyMarketTicker):
        ...
        mdm = DataManager(synthetic_size=Decimal('500'))
        event = PriceChangeEvent(ticker=test_ticker, price=Decimal('0.50'))
        mdm.process_price_change_event(event)
        ob = mdm.order_books[test_ticker]
    ...
    def test_update_order_book(
        self, market_data: DataManager, test_ticker: PolyMarketTicker
    ):
    ...
    def test_get_bids(self, market_data: DataManager, test_ticker: PolyMarketTicker):
        """Test getting bids."""
        order_book = OrderBook()
        order_book.update(
            asks=[],
    ...
    def test_get_asks(self, market_data: DataManager, test_ticker: PolyMarketTicker):
        """Test getting asks."""
        order_book = OrderBook()
        order_book.update(
            asks=[
...
class TestFindComplement:
    ...
    def test_no_bootstrap_skipped_without_no_token(self, market_data: DataManager):
