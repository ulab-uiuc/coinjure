    ...
    try:
        ...
        try:
            ...
            return (
                'tool_missing',
                None,
                'Neither `uv` nor `python -m pytest` is available.',
            )
...
def build_llm_prompt(
    ...
    lines.extend(
        [f'  - `{path}`' for path in inventory['references']] or ['  - (none)']
    )
    lines.extend(
        [
            '',
...
def combine_reviews(
    ...
    if llm_review and llm_review.get('provided'):
        ...
        lw_norm = lw / (aw + lw)
        final_score = int(
            round(auto_review['score'] * aw_norm + llm_review['score'] * lw_norm)
        )
...
def score_skill(
    ...
    skill_type = (
        'knowledge_only'
        if script_count == 0 and ref_count > 0
        else 'execution_or_hybrid'
    )
    ...
    if bash_blocks:
        ...
        unsafe_output_dir = any(
            '/scripts' in value or value.endswith('scripts')
    ...
    if not skill_uses_api:
        ...
        r'export\s+(FMP_API_KEY|FINVIZ_API_KEY|ALPACA_API_KEY)=', text
    ) or re.search(r'--api-key\s+\S+', text):
        exec_score += 4
    ...
    total_score = (
        metadata_score + workflow_score + exec_score + artifact_score + test_score
    )
...
def review_single_skill(
    ...
    if args.emit_llm_prompt:
        prompt_file = (
            output_dir
            / f"skill_review_prompt_{auto_review['skill_name']}_{timestamp}.md"
        )
...
def _run_all(
    ...
    for row in rows:
        ...
        print(
            f"{row['skill']:<40} {row['score']:>5} {status:>4} {row['high_findings']:>4}"
        )
    ...
    now = datetime.now()
    summary_path = (
        output_dir / f"skill_review_all_{now.strftime('%Y-%m-%d_%H%M%S')}.json"
    )
