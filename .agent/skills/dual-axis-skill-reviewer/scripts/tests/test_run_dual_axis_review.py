...
def test_discover_test_dirs_supports_two_layouts(reviewer_module, tmp_path: Path):
    ...
    write_text(
        skill_dir / 'scripts' / 'tests' / 'test_a.py',
        'def test_a():\n    assert True\n',
    )
...
def test_run_tests_fallbacks_to_python_pytest(
...
def test_run_tests_fallback_timeout_is_captured(
    reviewer_module, tmp_path: Path, monkeypatch
):
    skill_dir = tmp_path / 'skills' / 'sample'
    write_text(skill_dir / 'tests' / 'test_x.py', 'def test_x():\n    assert True\n')
...
def test_score_skill_counts_tests_in_root_tests_dir(reviewer_module, tmp_path: Path):
    ...
    write_text(
        skill_dir / 'tests' / 'test_sample.py', 'def test_sample():\n    assert True\n'
    )
...
def test_build_llm_prompt_includes_inventory(reviewer_module, tmp_path: Path):
    ...
    write_text(
        skill_dir / 'tests' / 'test_main.py', 'def test_main():\n    assert True\n'
    )
...
def test_execution_safety_max_25(reviewer_module, tmp_path: Path):
    ...
    assert (
        exec_safety <= 25
    ), f'execution_safety_reproducibility={exec_safety} exceeds max 25'
...
def test_api_key_score_exempt_with_frontmatter_flag(reviewer_module, tmp_path: Path):
    ...
    assert not any(
        'API key' in m for m in messages
    ), f'Unexpected API key finding: {messages}'
...
def test_combine_reviews_boundary_at_90_no_improvement(reviewer_module):
    ...
    merged = reviewer_module.combine_reviews(
        auto, None, auto_weight=1.0, llm_weight=0.0
    )
...
def test_main_e2e_generates_report_files(tmp_path: Path):
    ...
    report_files = list(
        (project_root / 'reports').glob('skill_review_e2e-skill_*.json')
    )
    assert report_files
...
def test_api_key_detected_from_scripts(reviewer_module, tmp_path: Path):
    ...
    no_api_review = reviewer_module.score_skill(
        project_root, no_api_md, skip_tests=True
    )
