...
def generate_markdown_report(analysis: dict, output_file: str):
    ...
    lines.append(
        f"| **Recommended Exposure** | {conviction.get('exposure_range', 'N/A')} |"
    )
    ...
    for key in optional_keys:
        ...
        if sig:
            ...
            key_signal = _format_key_signal(key, sig)
            lines.append(
                f"| {key.replace('_', ' ').title()} | {sig_score} | {key_signal} |"
            )
        else:
    ...
    lines.append('|-----------|-------|')
    lines.append(
        f"| Max Single Position | {sizing.get('max_single_position', 'N/A')}% |"
    )
    lines.append(
        f"| Daily Volatility Target | {sizing.get('daily_vol_target', 'N/A')}% |"
    )
...
def _format_key_signal(skill: str, sig: dict) -> str:
    ...
    if skill == 'market_breadth':
        ...
        return (
            f"State: {sig.get('state', 'N/A')}, Quality: {sig.get('quality_score', 0)}"
        )
