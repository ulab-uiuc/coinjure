...
def calculate_momentum(all_timeseries: list[dict], sector_summary: list[dict]) -> dict:
    ...
    accel_score, accel_value, accel_label = _score_acceleration_smoothed(
        smoothed_slopes
    )
    ...
    return {
        ...
        'data_available': True,
        'slope': current_slope,
        'slope_smoothed': round(smoothed_slope, 6)
...
def _score_sector_slope_breadth(sector_summary: list[dict]) -> tuple:
    ...
    positive = sum(
        1 for s in sector_summary if s.get('Slope') is not None and s['Slope'] > 0
    )
