...
def test_missing_total_score_in_eval_raises() -> None:
    history = {
        'strategy_id': 'x',
        'iterations': [
            {'iteration': 1, 'eval': {'dimensions': [], 'red_flags': [], 'inputs': {}}}
        ],
    }
...
def test_plateau_fires_when_scores_within_threshold(
    sample_iteration_history: dict,
) -> None:
...
def test_append_eval_increments_iteration(tmp_path: Path) -> None:
    ...
    result = ds.append_eval(
        eval_path, history_path, 'existing_strat', changes='Added filter'
    )
