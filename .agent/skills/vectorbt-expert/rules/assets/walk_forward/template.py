...
def run_ema_backtest(close_slice, fast_period, slow_period):
    ...
    ema_f = pd.Series(
        tl.EMA(close_slice.values, timeperiod=fast_period), index=close_slice.index
    )
    ema_s = pd.Series(
        tl.EMA(close_slice.values, timeperiod=slow_period), index=close_slice.index
    )
...
print(
    results_df[
        ...
        index=False, float_format=lambda x: f'{x:.2%}' if abs(x) < 10 else f'{x:.2f}'
    )
)
...
param_stable = (results_df['best_fast'].std() < 5) and (
    results_df['best_slow'].std() < 5
)
if param_stable:
