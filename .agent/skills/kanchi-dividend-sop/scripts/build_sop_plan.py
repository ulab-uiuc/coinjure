    ...
    if isinstance(raw, list):
        ...
        for item in raw:
            if isinstance(item, str):
                ...
                bucket = (
                    str(item.get('bucket', 'unassigned')).strip().lower()
                    or 'unassigned'
                )
                if ticker:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--input', help='Path to JSON input with candidates/tickers.', default=None
    )
    parser.add_argument('--tickers', help='Comma-separated ticker list.', default=None)
    parser.add_argument(
        '--output-dir', default='reports', help='Output directory path.'
    )
    parser.add_argument(
