...
def read_news_reactions(path: Path | None, as_of: date | None) -> list[dict[str, Any]]:
    ...
    if suffix == '.csv':
        ...
        if isinstance(payload, dict):
            data = (
                payload.get('rows') or payload.get('data') or payload.get('news') or []
            )
    ...
    filtered.sort(
        key=lambda item: abs(safe_float(item.get('reaction_1d'))), reverse=True
    )
    return filtered
...
def normalize_hint(raw_hint: dict[str, Any]) -> dict[str, Any]:
    ...
    title = str(
        raw_hint.get('title') or raw_hint.get('observation') or 'untitled_hint'
    ).strip()
...
def generate_llm_hints(
    llm_command: str | None, payload: dict[str, Any]
) -> list[dict[str, Any]]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--market-summary', default=None, help='Optional market_summary.json path'
    )
    parser.add_argument(
        '--anomalies', default=None, help='Optional anomalies.json path'
    )
    parser.add_argument(
        '--news-reactions',
        default=None,
    ...
    parser.add_argument(
        '--llm-ideas-cmd', default=None, help='Optional external LLM command'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.06,
        help='Min abs(reaction_1d) for hints',
    )
    parser.add_argument(
        '--max-total-hints', type=int, default=25, help='Max total hints to output'
    )
    parser.add_argument(
        '--output-dir',
        default='reports/',
...
def main() -> int:
    ...
    market_summary_path = (
        Path(args.market_summary).resolve() if args.market_summary else None
    )
    ...
    try:
        ...
        hints = dedupe_hints(
            rule_hints + llm_hints, max_total=max(args.max_total_hints, 0)
        )
