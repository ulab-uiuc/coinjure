...
class RepresentativeStockSelector:
    ...
    def _fetch_finviz_elite(
        ...
        filter_str = (
            f'ind_{ind_code},{cap_code},sh_avgvol_o100,sh_price_o10,{perf_code}'
        )
        url = (
    ...
    def _compute_composite_score(
        ...
        for i, s in enumerate(stocks):
            ...
            valid_counts = {
                'cap': n,
                'change': sum(1 for _, v in change_vals if v is not None)
                if has_change
                else 0,
                'vol': sum(1 for _, v in vol_vals if v is not None)
                if has_volume
                else 0,
            }
