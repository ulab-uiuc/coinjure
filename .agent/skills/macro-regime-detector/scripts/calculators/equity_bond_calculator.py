...
def calculate_equity_bond(spy_history: list[dict], tlt_history: list[dict]) -> dict:
    ...
    return {
        ...
        'correlation_6m': round(correlation_6m, 3)
        if correlation_6m is not None
        else None,
        'correlation_12m': round(correlation_12m, 3)
        if correlation_12m is not None
        else None,
        'crossover': crossover,
...
def _classify_correlation_regime(
    corr_6m: Optional[float], corr_12m: Optional[float]
) -> str:
