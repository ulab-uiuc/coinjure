...
def calculate_concentration(rsp_history: list[dict], spy_history: list[dict]) -> dict:
    ...
    if crossover['type'] in ('golden_cross', 'death_cross'):
        cross_dir = (
            'broadening' if crossover['type'] == 'golden_cross' else 'concentrating'
        )
        mom_dir = (
            'broadening'
...
def _describe_signal(
    ...
    if score >= 80:
        ...
        return (
            'TRANSITION SIGNAL: RSP/SPY crossover or sharp momentum reversal detected'
        )
    elif score >= 40:
