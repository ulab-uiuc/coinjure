...
def generate_markdown_report(analysis: dict, output_file: str):
    ...
    lines.append('|--------|-------|')
    lines.append(
        f'| **Current State** | {state_emoji} **{_state_label(combined_state)}** |'
    )
    ...
    for label, data in [('S&P 500', sp500), ('NASDAQ/QQQ', nasdaq)]:
        ...
        current = (
            f"${data['current_price']:.2f}" if data.get('current_price') else 'N/A'
        )
        ...
        sl_str = (
            f"{swing.get('date', 'N/A')} (${swing.get('price', 0):.2f})"
            if swing
            else 'None'
        )
    ...
    if has_rally:
        ...
        for label, data in [('S&P 500', sp500), ('NASDAQ/QQQ', nasdaq)]:
            ...
            lines.append(
                f"- **Current Day Count:** {rally.get('current_day_count', 0)}"
            )

            if rally.get('invalidated'):
                lines.append(
                    f"- **INVALIDATED:** {rally.get('invalidation_reason', 'N/A')}"
                )
            lines.append('')
    ...
    has_ftd = (sp_ftd and sp_ftd.get('ftd_detected')) or (
        nq_ftd and nq_ftd.get('ftd_detected')
    )
    ...
    if has_ftd:
        ...
        for label, ftd_data in [('S&P 500', sp_ftd), ('NASDAQ/QQQ', nq_ftd)]:
            ...
            lines.append(
                f"| **Day Number** | Day {ftd_data.get('ftd_day_number', 'N/A')} |"
            )
            lines.append(
    ...
    if post_dist or inv or pt:
        ...
        if pt:
            ...
            if pt.get('ema_21') is not None:
                ...
                lines.append(
                    f"  - 50 SMA Rising: {'Yes' if pt.get('sma_50_rising') else 'No'}"
                )
                lines.append(
    ...
    for label, data in [('S&P 500', sp500), ('NASDAQ/QQQ', nasdaq)]:
        ...
        if swing:
            lines.append(
                f"- Swing Low: ${swing.get('price', 0):.2f} ({swing.get('date', 'N/A')})"
            )
