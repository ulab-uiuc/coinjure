...
def main():
    ...
    for label, idx_data in [
        ('S&P 500', market_state['sp500']),
        ...
        if rally and rally.get('day1_date'):
            print(
                f"  {label} Rally Day 1: {rally['day1_date']} (Day {rally['current_day_count']})"
            )
    ...
    print(
        f"  Quality Score: {quality.get('total_score', 0)}/100 ({quality.get('signal', 'N/A')})"
    )
