...
def render_markdown(rows: list[PlanningRow], as_of: str) -> str:
    ...
    for row in rows:
        hold_days = (
            '' if row.hold_days_in_window is None else str(row.hold_days_in_window)
        )
        lines.append(
        ...
        for row in rows:
            writer.writerow(
                [
                    ...
                    row.instrument_type,
                    row.account_type,
                    row.hold_days_in_window
                    if row.hold_days_in_window is not None
                    else '',
                    row.classification,
                    row.location_hint,
                    row.note,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Build US dividend tax planning artifacts.'
    )
    parser.add_argument(
        '--input', required=True, help='Path to JSON input with holdings list.'
    )
