...
def calculate_price_gap(
    daily_prices: list[dict], earnings_date: str, timing: str
) -> float:
...
def validate_input_json(data: dict) -> list[dict]:
    ...
    for i, record in enumerate(results):
        ...
        if (
            not isinstance(record['earnings_date'], str)
            or len(record['earnings_date']) != 10
        ):
            errors.append('earnings_date must be YYYY-MM-DD string')
        if record['earnings_timing'] not in valid_timings:
            errors.append(
                f"earnings_timing '{record['earnings_timing']}' not in {valid_timings}"
            )
        if not isinstance(record['gap_pct'], (int, float)):
            errors.append(
                f"gap_pct must be numeric, got {type(record['gap_pct']).__name__}"
            )
...
def analyze_stock(
    ...
    pattern = analyze_weekly_pattern(
        weekly_candles, earnings_date, watch_weeks=watch_weeks
    )
...
def main():
    ...
    for i, candidate in enumerate(candidates):
        ...
        try:
            ...
            print(
                f'  WARNING: API budget exceeded at {symbol}. Processing collected data.'
            )
            break
...
def _get_candidates_mode_a(client: FMPClient, args) -> list[dict]:
    ...
    from_date = (datetime.now() - timedelta(days=args.lookback_days)).strftime(
        '%Y-%m-%d'
    )
