...
from pred_market_cli.trader.types import OrderStatus
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help='Path to engine control socket (default: ~/.pred-market-cli/engine.sock)',
)
def monitor(socket: str | None) -> None:
    ...
    the monitor.  Use ``pred-market-cli trade`` to control the engine.

    Examples:
        pred-market-cli monitor                       # attach to default socket
        pred-market-cli monitor -s /tmp/eng.sock      # custom socket path
        pred-market-cli trade status                  # check engine health
        pred-market-cli trade pause                   # pause LLM decisions
    """
    from pathlib import Path

    from pred_market_cli.cli.control import SOCKET_PATH
    from pred_market_cli.cli.textual_monitor import SocketTradingMonitorApp
