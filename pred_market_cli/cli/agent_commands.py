...
from pred_market_cli.live.live_trader import (
...
def _load_strategy_class(strategy_ref: str) -> type[Strategy]:
    ...
    if not isinstance(strategy_cls, type) or not issubclass(strategy_cls, Strategy):
        raise click.ClickException(
            f'Class {class_name!r} must inherit from pred_market_cli.strategy.strategy.Strategy'
        )
    return strategy_cls
...
from pred_market_cli.events.events import Event, NewsEvent, OrderBookEvent, PriceChangeEvent
from pred_market_cli.strategy.strategy import Strategy
