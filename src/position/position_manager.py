...
class PositionManager:
    ...
    def update_position(self, ticker: Ticker, quantity: Decimal, price: Decimal):
        """Update position
        Args:
            ticker: The ticker
        ...
        if quantity < 0:
            ...
            if sell_quantity > current_quantity:
                raise ValueError(f"Cannot sell {sell_quantity} {ticker}, exceeding the current position {current_quantity}")
    ...
    def get_unrealized_pnl(self, ticker: Ticker, market_data: MarketDataManager) -> Decimal:
        """Calculate unrealized PnL for a ticker"""
        if ticker.symbol not in self.positions:
            return Decimal('0')
        pos = self.positions[ticker.symbol]
        raise NotImplementedError("get_unrealized_pnl not implemented")
        #return (market_data.get_last_price(ticker) - pos.average_entry) * pos.quantity
    ...
    def get_pnl(self, ticker: Ticker, market_data: MarketDataManager) -> Decimal:
        ...
        return self.get_realized_pnl(ticker) + self.get_unrealized_pnl(ticker, market_data)
    ...
    def get_total_unrealized_pnl(self, market_data: MarketDataManager) -> Decimal:
        ...
        return sum(self.calculate_unrealized_pnl(pos.ticker, market_data) for pos in self.positions.values())
