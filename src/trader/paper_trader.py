...
class PaperTrader(Trader):
    ...
    def _simulate_execution(self, side: TradeSide, ticker: Ticker, limit_price: Decimal, quantity: Decimal) -> Trade | None:
    ...
    async def place_order(self, side: TradeSide, ticker: Ticker, limit_price: Decimal, quantity: Decimal) -> PlaceOrderResult:
        # Validate inputs
        ...
        # Check risk limits
        if not self.risk_manager.check_trade(ticker.symbol, side, quantity, limit_price):
            return PlaceOrderResult(
                status=OrderStatus.REJECTED,
