...
### Files Changed
- `swm_agent/data/data_source.py` — Add default `start()` / `stop()` lifecycle hooks
- `swm_agent/trader/trader.py` — Add `orders` attribute to base class
...
### Ping the Lead
> **PR-2: Interface additions (DataSource, Trader)**  
> [PR link]  
>  
...
### Files Changed
- `swm_agent/core/trading_engine.py` — EngineSnapshot, get_snapshot(), async stop(), exception backoff, request_stop()
- `swm_agent/data/live/live_data_source.py` — `_poll_task` handling, `stop()` for all three live sources
...
### Files Changed
- `swm_agent/cli/monitor.py` — Full-screen Rich TUI, 8 panels, TUILogHandler, DemoDataSource/DemoStrategy, --demo/--live/--paper/--real-trades
...
swm-agent --help
swm-agent monitor --help
swm-agent monitor --demo   # Run 10–15 s, then Ctrl+C
```
...
- Note: `swm-agent monitor` without `--live` now defaults to `--demo` (simulated data) instead of requiring a config. If external scripts relied on the old "single snapshot" default, they should use `--no-watch` explicitly.
...
> pip install -e .
> swm-agent monitor --help
> swm-agent monitor --demo   # Run 10–15 s, Ctrl+C — no traceback, no task leaks
> ```
...
- **Known risk:** `swm_agent/strategy/simple_strategy.py` line 89 contains a hardcoded API key. Per lead instruction, this file is **not modified** in any PR. No new secrets introduced.
