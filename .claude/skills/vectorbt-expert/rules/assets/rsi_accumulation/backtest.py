...
print(
    f'  NIFTYBEES 15m: {len(df_15m)} bars from {df_15m.index[0]} to {df_15m.index[-1]}'
)
...
print(
    f'  NIFTY Daily:   {len(df_nifty)} bars from {df_nifty.index[0]} to {df_nifty.index[-1]}'
)
...
# --- Benchmark ---
nifty_start = (
    df_nifty['close'].loc[df_nifty.index >= close_15m.index[0].normalize()].iloc[0]
)
...
print(
    f'  -> Buys more when market is fearful (low RSI) and less when greedy (high RSI)'
)
