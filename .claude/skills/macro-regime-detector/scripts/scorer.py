...
def classify_regime(component_results: dict[str, dict]) -> dict:
    ...
    available_count = sum(
        1
        for comp in [conc, yc, credit, size, eb, sector]
    ...
    corr_regime = (
        eb.get('correlation_regime', 'unknown')
        if eb.get('data_available', False)
        else 'unknown'
    )
    ...
    regime_scores = {
        ...
        'broadening': _score_broadening_regime(
            conc_dir, size_dir, credit_dir, sector_dir
        ),
        'contraction': _score_contraction_regime(
            credit_dir, sector_dir, eb_dir, size_dir
        ),
...
def _score_broadening_regime(
    conc_dir: str, size_dir: str, credit_dir: str, sector_dir: str
) -> int:
