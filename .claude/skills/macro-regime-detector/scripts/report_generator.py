...
def generate_markdown_report(analysis: dict, output_file: str):
    ...
    lines.append(
        f"| **Components Signaling** | {composite.get('signaling_components', 0)}/6 |"
    )
    ...
    if tied:
        lines.append(
            f"| **Competing Regimes** | {' vs '.join(r.capitalize() for r in tied)} |"
        )
    lines.append('')
    ...
    for i, key in enumerate(component_order, 1):
        ...
        comp_label = (
            composite.get('component_scores', {}).get(key, {}).get('label', key)
        )
        ...
        if key == 'yield_curve':
            if comp.get('steepening_type'):
                ...
                steep_label = steep_labels.get(
                    comp['steepening_type'], comp['steepening_type']
                )
    ...
    if tied and 'inflationary' in tied:
        lines.append(
            '> **Duration Warning:** Contraction and Inflationary signals are both'
        )
        lines.append(
            '> present. Long-duration Treasuries may not provide effective hedging'
        )
        ...
        if corr_regime == 'positive':
            lines.append(
                '- Correlation regime is **positive** -> short-duration Treasuries + TIPS'
            )
            ...
            lines.append(
                '- Correlation regime is **unknown** -> default to short-duration + TIPS'
            )
        lines.append('')
