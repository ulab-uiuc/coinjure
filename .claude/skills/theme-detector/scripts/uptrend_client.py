...
SECTOR_SUMMARY_URL = 'https://raw.githubusercontent.com/tradermonty/uptrend-dashboard/main/data/sector_summary.csv'
...
def fetch_sector_uptrend_data() -> dict[str, dict]:
    ...
    for ws_name, rows in sector_rows.items():
        ...
        slope = (
            latest['slope_csv'] if latest['slope_csv'] is not None else calculated_slope
        )
...
def get_sector_uptrend_3point(
    sector_name: str, all_data: dict[str, dict]
) -> Optional[dict]:
