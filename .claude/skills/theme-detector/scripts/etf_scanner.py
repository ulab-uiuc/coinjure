...
try:
    ...
    print(
        'ERROR: pandas/numpy not found. Install with: pip install pandas numpy',
        file=sys.stderr,
    )
    sys.exit(1)
...
class ETFScanner:
    ...
    def _fetch_fmp_historical(
        ...
        missing = [
            s for s in symbols if self._normalize_symbol_for_fmp(s) not in result
        ]
        for s in missing:
            norm = self._normalize_symbol_for_fmp(s)
            ...
            if 'historicalStockList' in data:
                for entry in data['historicalStockList']:
                    ...
                    if sym and 'historical' in entry:
                        result[self._normalize_symbol_for_fmp(sym)] = entry[
                            'historical'
                        ]
            ...
            for m in yf_list:
                ...
                if sym in fmp_results:
                    ...
                    for k, v in m.items():
                        if (
                            k != 'symbol'
                            and fmp_results[sym].get(k) is None
                            and v is not None
                        ):
                            fmp_results[sym][k] = v
                else:
