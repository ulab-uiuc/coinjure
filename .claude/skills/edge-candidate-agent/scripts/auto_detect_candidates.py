...
def parse_as_of_date(raw_as_of: str | None) -> date | None:
    ...
    try:
        ...
        raise AutoDetectError(
            f'invalid --as-of format (expected YYYY-MM-DD): {raw_as_of}'
        ) from exc
...
def normalize_hints(raw_hints: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for idx, hint in enumerate(raw_hints):
        ...
        title = str(
            hint.get('title') or hint.get('observation') or f'hint_{idx + 1}'
        ).strip()
...
def score_breakout_candidate(
    ...
    regime_component = {'RiskOn': 10.0, 'Neutral': 5.0, 'RiskOff': -5.0}.get(
        regime_label, 0.0
    )
...
def score_gap_candidate(
    ...
    regime_component = {'RiskOn': 8.0, 'Neutral': 4.0, 'RiskOff': -8.0}.get(
        regime_label, 0.0
    )
...
def score_reversal_candidate(
    ...
    regime_component = {'RiskOff': 12.0, 'Neutral': 7.0, 'RiskOn': 2.0}.get(
        regime_label, 0.0
    )
...
def build_ticket_payload(
    ...
    hypothesis_type = str(
        ...
        or (
            ENTRY_FAMILY_TO_HYPOTHESIS.get(entry_family)
            if entry_family
            else 'regime_shift'
        )
    )
    ...
    if entry_family in FAMILY_NAME:
        ...
        description = (
            f'Auto-detected {FAMILY_NAME[entry_family]} candidate for {symbol}.'
        )
    else:
        ...
        description = str(
            ...
            or f'Auto-detected {hypothesis_tag} research candidate.'
        )
...
def compute_features(
    ...
    if target_date not in available_dates:
        ...
        if not prior_dates:
            raise AutoDetectError(
                f'no OHLCV data at or before as_of={target_date.isoformat()}'
            )
    ...
    frame['ma20'] = group['close'].transform(
        lambda s: s.rolling(20, min_periods=20).mean()
    )
    frame['ma50'] = group['close'].transform(
        lambda s: s.rolling(50, min_periods=50).mean()
    )
    frame['ma200'] = group['close'].transform(
        lambda s: s.rolling(200, min_periods=200).mean()
    )

    frame['vol_avg20'] = group['volume'].transform(
        lambda s: s.rolling(20, min_periods=20).mean()
    )
    ...
    frame['atr14'] = group['tr'].transform(
    ...
    for metric in ('ret_1d', 'rel_volume', 'gap', 'atr_pct'):
        rolling_mean = group[metric].transform(
            lambda s: s.rolling(60, min_periods=30).mean()
        )
        rolling_std = group[metric].transform(
            lambda s: s.rolling(60, min_periods=30).std(ddof=0)
        )
        frame[f'{metric}_z'] = (frame[metric] - rolling_mean) / rolling_std.replace(
            0, np.nan
        )
    ...
    if latest.empty:
        raise AutoDetectError(
            f'no rows found for target date {target_date.isoformat()}'
        )
...
def compute_regime(
    ...
    if tradable.empty:
        raise AutoDetectError(
            'no tradable symbols remain after min price/volume filters'
        )
    ...
    recent = full_frame[full_frame['date'].isin(last_20_dates)][
        ['date', 'symbol', 'ret_1d']
    ].copy()
    ...
    if not spy_today.empty:
        ...
        spy_above_ma200 = (
            bool(spy_close > spy_ma200)
            if not math.isnan(spy_ma200)
            else spy_above_ma200
        )
        spy_ret_1d = (
    ...
    summary = {
        ...
        'avg_pair_corr_20': None
        if math.isnan(avg_pair_corr)
        else round(avg_pair_corr, 4),
        ...
        candidates.append(
            {
                ...
                'trend_filter': [
                    'avoid fresh breakdown regimes unless reversal setup confirms'
                ],
                'rationale': [
...
def scan_regime_shift_candidate(
    ...
    daily = (
        ...
        .agg(
            pct_above_ma50=(
                'close',
                lambda s: float((s > valid.loc[s.index, 'ma50']).mean()),
            ),
            pct_above_ma200=(
                'close',
                lambda s: float((s > valid.loc[s.index, 'ma200']).mean()),
            ),
            avg_ret=('ret_1d', 'mean'),
        )
        .dropna()
    ...
    breadth_delta = float(
        tail['pct_above_ma50'].iloc[-1] - tail['pct_above_ma50'].iloc[0]
    )
    ...
    if breadth_delta >= 0.08 and ret_5d > 0 and float(vol_trend) <= 1.05:
        ...
        description = 'Breadth deterioration with rising volatility, possible early RiskOff transition.'
    else:
        return []
...
def scan_correlation_chain_candidates(
    ...
    dv = recent.groupby('symbol')['dollar_volume'].apply(
        lambda s: float(s.tail(20).mean())
    )
    ...
    for leader_symbol, leader_row in leaders.head(6).iterrows():
        ...
        for follower_symbol, lag_corr in followers.items():
            ...
            score = (
                ...
                + (
                    10.0
                    if float(follower_today['close']) > float(follower_today['ma50'])
                    else 4.0
                )
            )
            candidates.append(
    ...
    for candidate in sorted(
        candidates, key=lambda item: item['priority_score'], reverse=True
    ):
...
def scan_calendar_anomaly_candidates(
    ...
    candidates: list[dict[str, Any]] = []
    for symbol, row in (
        stats.sort_values('mean_ret', ascending=False).head(20).iterrows()
    ):
...
def scan_news_reaction_candidates(
    ...
    if not required.issubset(frame.columns):
        raise AutoDetectError(
            'news reactions file must include symbol,timestamp,reaction_1d'
        )
    ...
    for _, row in (
        day.sort_values('abs_reaction', ascending=False).head(max(top_n, 0)).iterrows()
    ):
        ...
        out.append(
            {
                ...
                'trend_filter': [
                    'validate with follow-through and volume confirmation'
                ],
                'rationale': [
...
def scan_futures_trigger_candidates(
    ...
    frame['ret_z'] = frame.groupby('symbol')['ret_1d'].transform(
        lambda s: (
            (s - s.rolling(60, min_periods=30).mean())
            ...
            candidates.append(
                {
                    ...
                    'conditions': [
                        f'future={fut_symbol}',
                        f'future_ret_z={round(shock_z, 3)}',
                    ],
                    'trend_filter': [
                        'validate cross-asset sensitivity before deployment'
                    ],
                    'rationale': [
...
def write_outputs(
    ...
    report_markdown = render_daily_report(
        ...
        anomalies=anomalies,
        exportable_tickets=[
            yaml.safe_load(path.read_text()) for path in exportable_paths
        ],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--top-n', type=int, default=10, help='Number of top tickets to generate'
    )
    parser.add_argument(
        '--anomaly-top-k', type=int, default=10, help='Number of anomalies to keep'
    )
    parser.add_argument(
        '--min-price', type=float, default=5.0, help='Minimum close price filter'
    )
    parser.add_argument(
        '--min-avg-volume',
        type=float,
    ...
    parser.add_argument(
        '--force-export',
        action='store_true',
        help='Overwrite exported strategy artifacts',
    )
    parser.add_argument(
        '--pipeline-root',
        default=None,
...
def main() -> int:
    ...
    if args.export_strategies_dir:
        ...
        pipeline_root = (
            Path(args.pipeline_root).resolve() if args.pipeline_root else None
        )
        try:
