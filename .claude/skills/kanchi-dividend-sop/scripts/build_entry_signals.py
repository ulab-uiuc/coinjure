...
def normalize_metrics_yields(
    metrics: list[dict[str, Any]], max_points: int = 5
) -> list[float]:
...
def build_entry_row(
    ...
    avg_yield_5y_pct = (
        round(avg_yield_5y_pct_raw, 2) if avg_yield_5y_pct_raw is not None else None
    )
    ...
    if (
        ...
        and target_yield_pct is not None
        and target_yield_pct > 0
    ):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Build Kanchi Step 5 entry signals from FMP data.'
    )
    parser.add_argument(
        '--input', default=None, help='Path to JSON file containing tickers.'
    )
    ...
    parser.add_argument(
        '--output-dir', default='reports', help='Directory for outputs.'
    )
    parser.add_argument(
        '--as-of',
