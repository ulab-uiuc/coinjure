...
def generate_markdown_report(
    ...
    mode_desc = (
        'FMP Earnings Calendar' if mode == 'A' else 'Earnings Trade Analyzer JSON'
    )
    ...
    if api_stats:
        ...
        lines.append(
            f"- **Budget Remaining:** {api_stats.get('budget_remaining', 'N/A')}"
        )
    ...
    lines.append('')
    lines.append(
        'This screener identifies Post-Earnings Announcement Drift (PEAD) setups:'
    )
    lines.append('')
    lines.append(
        '1. **Setup Quality** (30%) - Earnings gap quality and weekly pattern formation'
    )
    lines.append(
...
def _format_stock_entry(rank: int, stock: dict) -> list[str]:
    ...
    # Composite score
    lines.append(
        f"**PEAD Score:** {stock.get('composite_score', 0):.1f}/100 ({rating})"
    )
    lines.append('')
    ...
    if stage == 'BREAKOUT':
        ...
        lines.append(
            f'- Entry: ${entry:.2f} (breakout +{breakout_pct:.1f}% above red candle high)'
        )
        ...
        if red_candle:
            lines.append(
                f"- Trigger: Close above ${red_candle['high']:.2f} on green weekly candle"
            )
