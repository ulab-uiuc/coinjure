...
def _make_triggers(trigger_ids: list[str]) -> list[dict]:
    ...
    return [
        {'trigger': t, 'severity': 'high', 'message': f'Trigger {t} fired'}
...
class TestGenerateInversions:
    def test_generate_inversions_cost_defeat(self):
        ...
        assert (
            has_short_horizon
        ), 'cost_defeat should produce at least one short-horizon proposal'
    ...
    def test_generate_inversions_tail_risk(self):
        ...
        assert (
            has_tight_risk
        ), 'tail_risk should produce at least one tight-risk proposal'
...
class TestRankAndSelect:
    ...
    def test_rank_and_select_diversity_constraint(self):
        ...
        assert len(archetypes) == len(
            set(archetypes)
        ), 'Each archetype should appear at most once'
    ...
    def test_rank_and_select_tiebreak_deterministic(self):
        ...
        results = []
        for _ in range(5):
            selected = gp.rank_and_select(
                [p_a, p_b, p_c], draft, triggers, max_pivots=10
            )
        ...
        for i in range(len(selected) - 1):
            ...
            if s_i['combined'] == s_j['combined']:
                if s_i['novelty'] == s_j['novelty']:
                    assert (
                        selected[i]['id'] < selected[i + 1]['id']
                    ), 'Same combined and novelty -> alphabetical id order'
                else:
                    assert (
                        s_i['novelty'] >= s_j['novelty']
                    ), 'Same combined -> higher novelty first'
...
class TestCrossValidation:
    ...
    def test_required_validation_fields_consistent(self):
        ...
        our_fields = {
            e.split('.')[1].split(' ')[0] for e in our_errors if 'must be' in e
        }
        their_fields = {
            e.split('.')[1].split(' ')[0] for e in their_errors if 'must be' in e
        }
