...
def t3_credit_stress_proxy(holding: dict[str, Any]) -> TriggerFinding | None:
    ...
    interest_coverage_history = float_history(
        balance_sheet.get('interest_coverage_history')
    )

    buybacks = to_float(capital.get('buybacks')) or 0.0
    dividends_paid = (
        to_float(capital.get('dividends_paid'))
        or to_float(cashflow.get('dividends_paid'))
        or 0.0
    )
    ...
    icov_weak = (
        len(interest_coverage_history) >= 1 and interest_coverage_history[-1] < 2.5
    )
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for item in report['results']:
        ...
        lines.append(
            f"| {item['ticker']} | {item['status']} | {triggers} | {actions} |"
        )
    ...
    for item in report['results']:
        ...
        for finding in item['findings']:
            lines.append(
                f"- `{finding['trigger']}` `{finding['status']}`: {finding['reason']}"
            )
        lines.append('')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--output', help='Path to output JSON report (overrides --output-dir).'
    )
    parser.add_argument(
        '--markdown',
