...
class FINVIZClient:
    ...
    def screen_stocks(self) -> set[str]:
        ...
        try:
            print(
                'Fetching pre-screened stocks from FINVIZ Elite API...', file=sys.stderr
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f'✅ FINVIZ returned {len(symbols)} pre-screened stocks',
                    file=sys.stderr,
                )
                return symbols
                ...
                print(
                    'ERROR: FINVIZ API authentication failed. Check your API key.',
                    file=sys.stderr,
                )
                ...
                print(
                    f'ERROR: FINVIZ API request failed: {response.status_code}',
                    file=sys.stderr,
                )
                return set()
...
class StockAnalyzer:
    ...
    def analyze_dividend_growth(
        ...
        for div in dividends:
            year = div['date'][:4]
            annual_dividends[year] = annual_dividends.get(year, 0) + div.get(
                'dividend', 0
            )
    ...
    def analyze_revenue_growth(
        ...
        cagr = (
            StockAnalyzer.calculate_cagr(revenues[0], revenues[-1], 3)
            if revenues[0] > 0
            else None
        )
    ...
    def analyze_dividend_sustainability(
        ...
        if result['payout_ratio'] and result['fcf_payout_ratio']:
            result['sustainable'] = (
                result['payout_ratio'] < 80 and result['fcf_payout_ratio'] < 100
            )
    ...
    def analyze_financial_health(balance_sheets: list[dict]) -> dict:
        ...
        if result['debt_to_equity'] is not None and result['current_ratio'] is not None:
            result['healthy'] = (
                result['debt_to_equity'] < 2.0 and result['current_ratio'] > 1.0
            )
    ...
    def analyze_revenue_trend(income_statements: list[dict]) -> dict:
        ...
        if revenues_oldest_first[0] > 0 and revenues_oldest_first[-1] > 0:
            ...
            if years > 0:
                cagr = (
                    pow(revenues_oldest_first[-1] / revenues_oldest_first[0], 1 / years)
                    - 1
                ) * 100
    ...
    def analyze_earnings_trend(income_statements: list[dict]) -> dict:
        ...
        if earnings_oldest_first[0] > 0 and earnings_oldest_first[-1] > 0:
            ...
            if years > 0:
                cagr = (
                    pow(earnings_oldest_first[-1] / earnings_oldest_first[0], 1 / years)
                    - 1
                ) * 100
                result['cagr'] = round(cagr, 2)
    ...
    def calculate_quality_score(
        key_metrics: list[dict], income_statements: list[dict]
    ) -> dict:
        ...
        if result['profit_margin']:
            score += min(
                result['profit_margin'] / 15 * 50, 50
            )  # Max 50 points for 15%+ margin
...
def screen_value_dividend_stocks(
    ...
    if finviz_symbols:
        ...
        candidates = []
        print(
            'Fetching quote and profile data from FMP for FINVIZ symbols...',
            file=sys.stderr,
        )
    ...
    print(
        'Note: Analysis will continue until API rate limit is reached', file=sys.stderr
    )
    ...
    for i, stock in enumerate(candidates, 1):  # Analyze all candidates until rate limit
        ...
        print(
            f'[{i}/{len(candidates)}] Analyzing {symbol} - {company_name}...',
            file=sys.stderr,
        )
        ...
        if client.rate_limit_reached:
            print(
                f'\n⚠️  API rate limit reached after analyzing {i - 1} stocks.',
                file=sys.stderr,
            )
            print(
        ...
        if rsi is None:
            print(
                '  ⚠️  RSI calculation failed (insufficient price data)', file=sys.stderr
            )
            continue
        ...
        if actual_dividend_yield < 3.0:
            print(
                f'  ⚠️  Dividend yield {actual_dividend_yield:.2f}% < 3.0%',
                file=sys.stderr,
            )
            continue
        ...
        if (
            dividend_stability['volatility_pct']
            and dividend_stability['volatility_pct'] > 100
        ):
            # Allow if consistently growing despite volatility
            if (
                not dividend_stability['is_growing']
                or dividend_stability['years_of_growth'] < 3
            ):
                print(
        ...
        composite_score += min(
            div_cagr / 10 * 15, 15
        )  # Max 15 points for 10%+ div growth
        composite_score += (
            stability_score * 0.2
        )  # Max 20 points from stability (100 * 0.2)
        composite_score += min(
            (revenue_cagr or 0) / 10 * 10, 10
        )  # Max 10 points for revenue
        ...
        result = {
            ...
            'roe': round(key_metrics[0].get('roe', 0) * 100, 1)
            if key_metrics
            else None,
    ...
    if oversold_results:
        print(
            f'\nStep 3: Found {len(oversold_results)} oversold stocks (RSI <= 40)',
            file=sys.stderr,
        )
