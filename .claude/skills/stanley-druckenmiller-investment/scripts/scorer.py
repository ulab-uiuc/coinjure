        ...
        'component_scores': {
            k: {
                ...
                'weighted_contribution': round(
                    raw_scores.get(k, 0) * effective_weights[k], 1
                ),
...
def classify_pattern(
    signals: dict, component_scores: dict, conviction_score: float
) -> dict:
    """
    ...
    conv_score = (
        convergence.get('score', 50) if isinstance(convergence, dict) else convergence
    )
