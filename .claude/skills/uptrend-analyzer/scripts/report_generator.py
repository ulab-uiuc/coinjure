...
def generate_json_report(analysis: dict, output_file: str):
    ...
    try:
        ...
        print(
            f'ERROR: Failed to write JSON report to {output_file}: {e}', file=sys.stderr
        )
        raise
...
def generate_markdown_report(analysis: dict, output_file: str):
    ...
    if warning_penalty != 0:
        ...
        lines.append(
            f'| **Warning Penalty** | {warning_penalty} (raw: {raw_score}/100) |'
        )
    ...
    if rotation.get('data_available'):
        ...
        lines.append(
            f"- **Defensive Avg:** {rotation.get('defensive_avg_pct', 'N/A')}%"
        )
        lines.append(
            f"- **Commodity Avg:** {rotation.get('commodity_avg_pct', 'N/A')}%"
        )
        lines.append(
            f"- **Cyclical-Defensive Gap:** {rotation.get('difference_pct', 'N/A')}pp"
        )
    ...
    if historical.get('data_available'):
        lines.append(
            f"- **Current Ratio:** {historical.get('current_ratio_pct', 'N/A')}%"
        )
        ...
        lines.append(
            f"- **Historical Median:** {historical.get('historical_median_pct', 'N/A')}%"
        )
    ...
    sector_details = components.get('sector_participation', {}).get(
        'sector_details', []
    )
    if sector_details:
        lines.append('---')
        ...
        divergent = [
            ...
            if (
                s.get('status') == 'Overbought' and s.get('trend', '').lower() == 'down'
            )
    ...
    try:
        ...
        print(
            f'ERROR: Failed to write Markdown report to {output_file}: {e}',
            file=sys.stderr,
        )
        raise
