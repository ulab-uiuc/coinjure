...
def calculate_market_breadth(
    latest_all: Optional[dict], all_timeseries: list[dict]
) -> dict:
    """
    ...
    distance_from_upper = (
        round(ratio - UPPER_THRESHOLD, 4) if ratio is not None else None
    )
    distance_from_lower = (
        round(ratio - LOWER_THRESHOLD, 4) if ratio is not None else None
    )
