...
def calculate_sector_rotation(
    ...
    if not sector_summary:
        if sector_timeseries:
            ...
            print(
                '  (fallback: built sector summary from timeseries data)',
                file=sys.stderr,
            )
        else:
            return {
    ...
    return {
        ...
        'commodity_avg_pct': round(commodity_avg * 100, 1)
        if commodity_avg is not None
        else None,
...
def _get_group_ratios(
    sector_map: dict[str, dict], sector_names: list[str]
) -> list[float]:
...
def _analyze_group(sector_map: dict[str, dict], sector_names: list[str]) -> dict:
    ...
    for _i, (name, ratio) in enumerate(zip(names_with_data, ratios)):
        if abs(ratio - mean) > 1.5 * std_dev and std_dev > 0:
            outliers.append(
                {'sector': name, 'ratio': ratio, 'deviation': round(ratio - mean, 4)}
            )
    ...
    if trends:
        ...
        for name, trend in zip(names_with_data, trends):
            if trend and trend != majority:
                trend_dissenters.append(
                    {'sector': name, 'trend': trend, 'majority': majority}
                )
