...
def calculate_composite_score(
    ...
    components = {
        ...
        'Volume Trend': {
            'score': volume_score,
            'weight': COMPONENT_WEIGHTS['volume_trend'],
        },
        'MA200 Position': {
            'score': ma200_score,
            'weight': COMPONENT_WEIGHTS['ma200_position'],
        },
        'MA50 Position': {
            'score': ma50_score,
            'weight': COMPONENT_WEIGHTS['ma50_position'],
        },
    }

    composite_score = sum(
        comp['score'] * comp['weight'] for comp in components.values()
    )
