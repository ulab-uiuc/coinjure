...
def _make_prices(
    n, start=100.0, daily_change=0.0, volume=1000000, start_date='2025-01-01'
):
...
class TestReportGenerator:
    ...
    def test_json_uses_all_results_for_summary(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            all_results = [
                self._make_result(
                    f'S{i}', score=90 - i * 5, grade='A' if i == 0 else 'B'
                )
                for i in range(10)
            ]
            ...
            generate_json_report(
                top_results, metadata, json_path, all_results=all_results
            )
            with open(json_path) as f:
                data = json.load(f)
