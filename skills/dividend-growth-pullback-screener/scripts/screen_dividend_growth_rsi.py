...
class FMPClient:
    ...
    def screen_stocks(
        self, min_market_cap: int = 2000000000, exchange: str = None
    ) -> list[dict]:
    ...
    def get_quote_with_profile(self, symbol: str) -> Optional[dict]:
        ...
        if profile:
            ...
            quote_data['companyName'] = profile.get(
                'companyName', quote_data.get('name', '')
            )
            quote_data['industry'] = profile.get('industry', '')
        else:
            # Fallback if profile fetch fails
            quote_data['sector'] = quote_data.get('sector', 'Unknown')
            quote_data['companyName'] = quote_data.get(
                'name', quote_data.get('companyName', '')
            )
...
class StockAnalyzer:
    ...
    def calculate_payout_ratios(
        ...
        if is_reit:
            result['payout_ratio'] = StockAnalyzer.calculate_ffo_payout_ratio(
                cash_flows
            )
        else:
            ...
            if income_stmts:
                ...
                if net_income > 0 and dividends_paid > 0:
                    result['payout_ratio'] = round(
                        (dividends_paid / net_income) * 100, 1
                    )
    ...
    def analyze_financial_health(balance_sheet: list[dict]) -> dict:
        ...
        debt_to_equity = (
            round(total_debt / total_equity, 2) if total_equity > 0 else None
        )
        current_ratio = (
...
def screen_dividend_growth_pullbacks(
    ...
    if max_candidates and not finviz_symbols:
        ...
        print(
            f'Limiting analysis to first {max_candidates} candidates', file=sys.stderr
        )

    print('\nStep 2: Detailed analysis of candidates...', file=sys.stderr)
    print(
        'Note: Analysis will continue until API rate limit is reached\n',
        file=sys.stderr,
    )
    ...
    for i, stock in enumerate(candidates, 1):
        ...
        if not div_cagr or div_cagr < min_div_growth:
            print(
                f'  ⚠️  Dividend CAGR {div_cagr}% < {min_div_growth}%', file=sys.stderr
            )
            continue
        ...
        print(
            f'  ✓ Dividend: {actual_dividend_yield:.2f}% yield, {div_cagr}% CAGR',
            file=sys.stderr,
        )
        ...
        growth_metrics = analyzer.analyze_growth_metrics(
        ...
        health_metrics = analyzer.analyze_financial_health(
            balance_sheet if balance_sheet else []
        )
...
Environment Variables:
  ...
  FINVIZ_API_KEY    - FINVIZ Elite API key (required for --use-finviz)
    ...
    print(f"\n{'=' * 80}", file=sys.stderr)
    print(
        f'Screening complete! Found {len(results)} qualified stocks.', file=sys.stderr
    )
    print(f"{'=' * 80}\n", file=sys.stderr)
