...
pf_bench = vbt.Portfolio.from_holding(
    bench_close, init_cash=INIT_CASH, fees=FEES, freq='1D'
)
...
print(
    f"  -> This measures return per unit of risk. Think of it as 'bang for your buck'."
)
print(f'     Below 1 = not enough return for the risk taken')
print(f'     1-2 = decent, 2+ = very good')
print(f'* Max Drawdown: {pf.max_drawdown() * 100:.2f}%')
print(
    f'  -> The biggest drop from peak. If you invested 10L, you would have temporarily'
)
print(
    f'     lost up to Rs {abs(pf.max_drawdown()) * INIT_CASH:,.0f} before recovering.'
)
print(f'* Win Rate: {pf.trades.win_rate() * 100:.1f}% of trades were profitable')
print(f'* Profit Factor: {pf.trades.profit_factor():.2f}')
print(
    f'  -> For every Rs 1 lost, you made Rs {pf.trades.profit_factor():.2f} in profits.'
)
