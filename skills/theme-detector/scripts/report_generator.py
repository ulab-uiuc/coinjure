...
def generate_json_report(
    ...
    data_quality = _assess_data_quality(
        themes, industry_rankings, sector_uptrend, metadata
    )
...
def generate_markdown_report(json_data: dict, top_n_detail: int = 3) -> str:
    ...
    if uptrend:
        ...
        for sector_name, data in sorted(uptrend.items()):
            ...
            ma_pct = (
                f"{data['ma_10'] * 100:.1f}%"
                if data.get('ma_10') is not None
                else 'N/A'
            )
            slope_str = (
                f"{data['slope']:+.4f}" if data.get('slope') is not None else 'N/A'
            )
            lines.append(
                f"| {sector_name} "
                f"| {ratio_pct} "
...
def _assess_data_quality(
    ...
    if not sector_uptrend:
        flags.append(
            'Sector uptrend data unavailable - GitHub CSV fetch may have failed'
        )
    else:
...
def _add_theme_details(lines: list[str], themes: list[dict]) -> None:
    ...
    for t in themes:
        ...
        lines.append(
            f"- **Heat:** {t.get('heat', 0):.1f}/100 ({t.get('heat_label', 'N/A')})"
        )
