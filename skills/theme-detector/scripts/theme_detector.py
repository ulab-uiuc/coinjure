...
def main():
    ...
    print(
        f"  FMP API: {'available' if fmp_available else 'not available'}",
        file=sys.stderr,
    )
    ...
    def _theme_priority(t):
        ...
        avg_strength = sum(abs(ind.get('weighted_return', 0)) for ind in inds) / max(
            len(inds), 1
        )
    ...
    if sector_uptrend:
        ...
        if stale_data:
            print(
                f'  WARNING: Uptrend data is stale (latest: {latest_date})',
                file=sys.stderr,
            )
