...
class TestClusterByProximity:
    def test_adjacent_within_both_thresholds_grouped(self):
        ...
        clusters = _cluster_by_proximity(
            industries, gap_threshold=3.0, vector_threshold=0.5
        )
        assert len(clusters) >= 1
        cluster_names = [set(i['name'] for i in c) for c in clusters]
        assert {'A', 'B'} in cluster_names or any(
            'A' in c and 'B' in c for c in cluster_names
        )
    ...
    def test_vector_distance_exceeds_splits(self):
        ...
        clusters = _cluster_by_proximity(
            industries, gap_threshold=3.0, vector_threshold=0.3
        )
...
class TestDiscoverThemes:
    ...
    def test_single_sector_with_no_vertical_overlap_kept(self):
        ...
        if discovered:
            names = {
                i['name'] for d in discovered for i in d.get('matching_industries', [])
            }
