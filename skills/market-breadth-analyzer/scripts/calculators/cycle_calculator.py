...
def calculate_cycle_position(rows: list[dict]) -> dict:
    ...
    if len(rows) >= 6:
        ma8_trend = (
            'rising' if current_8ma > rows[-6]['Breadth_Index_8MA'] else 'falling'
        )
    else:
        ma8_trend = 'unknown'
...
def _generate_signal(
    ...
    if marker_type == 'TROUGH':
        if days_since <= 20:
            phase = (
                'early recovery' if ma8_trend == 'rising' else 'failed reversal attempt'
            )
        elif days_since <= 60:
            phase = (
                'sustained recovery' if ma8_trend == 'rising' else 'stalled recovery'
            )
        else:
