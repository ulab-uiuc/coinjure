...
class TestBaseScore:
    ...
    def test_signal_on_trend_up(self, make_row):
        rows = [
            make_row(Bearish_Signal=True, Breadth_200MA_Trend=1, Breadth_Index_8MA=0.40)
        ]
        result = calculate_bearish_signal(rows)
    ...
    def test_signal_on_trend_down(self, make_row):
        rows = [
            make_row(
                Bearish_Signal=True, Breadth_200MA_Trend=-1, Breadth_Index_8MA=0.40
            )
        ]
        result = calculate_bearish_signal(rows)
...
class TestContextAdjustment:
    ...
    def test_signal_on_high_8ma_gets_plus15(self, make_row):
        rows = [
            make_row(Bearish_Signal=True, Breadth_200MA_Trend=1, Breadth_Index_8MA=0.55)
        ]
        result = calculate_bearish_signal(rows)
    ...
    def test_signal_on_low_8ma_gets_minus5(self, make_row):
        rows = [
            make_row(
                Bearish_Signal=True, Breadth_200MA_Trend=-1, Breadth_Index_8MA=0.20
            )
        ]
        result = calculate_bearish_signal(rows)
    ...
    def test_no_adjustment_when_signal_off(self, make_row):
        rows = [
            make_row(
                Bearish_Signal=False, Breadth_200MA_Trend=1, Breadth_Index_8MA=0.55
            )
        ]
        result = calculate_bearish_signal(rows)
