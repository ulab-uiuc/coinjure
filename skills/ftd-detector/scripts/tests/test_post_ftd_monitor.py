...
def _build_post_ftd_bars(
    ftd_close=100.0, ftd_low=99.5, ftd_volume=1_000_000, post_days=None
):
    """
    ...
    bars.append(
        make_bar(
            ftd_close * 0.99, volume=ftd_volume, date='pre-ftd', low=ftd_close * 0.985
        )
    )
    # FTD bar
    ...
    for i, (close, volume) in enumerate(post_days):
        bars.append(
            make_bar(close, volume=volume, date=f'post-{i + 1}', low=close * 0.997)
        )
