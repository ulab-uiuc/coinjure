            ...
            distributions.append(
                {
                    ...
                    'volume_change_pct': round(
                        (curr_volume / prev_volume - 1) * 100, 1
                    ),
                }
            )
...
def calculate_ftd_quality_score(market_state: dict) -> dict:
    ...
    if days_monitored == 0:
        ...
        health_adj = 10
        breakdown['post_ftd'] = (
            f'No post-FTD distribution ({days_monitored} days clean): +10'
        )
    elif earliest_dist is not None and earliest_dist <= 2:
        health_adj = -30
        breakdown['post_ftd'] = (
            f'Distribution on Day {earliest_dist}: -30 (very bearish)'
        )
        ...
        health_adj = -5
        breakdown['post_ftd'] = (
            f'Distribution on Day {earliest_dist}: -5 (mild negative)'
        )
    else:
        health_adj = 0
