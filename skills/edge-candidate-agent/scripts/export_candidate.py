...
def build_strategy_spec(ticket: dict[str, Any], candidate_id: str) -> dict[str, Any]:
    ...
    if 'validation' in ticket:
        ...
        if validation.get('method', 'full_sample') != 'full_sample':
            raise ExportError(
                "ticket.validation.method must be 'full_sample' for Phase I export"
            )
    ...
    spec: dict[str, Any] = {
        'id': candidate_id,
        'name': ticket.get(
            'name', candidate_id.replace('_', ' ').replace('-', ' ').title()
        ),
        ...
        'signals': {
            'entry': {
                'type': entry_family,
                'conditions': entry_overrides.get(
                    'conditions', entry_defaults['conditions']
                ),
                'trend_filter': entry_overrides.get(
                    'trend_filter', entry_defaults['trend_filter']
                ),
            },
        ...
        'promotion_gates': deep_merge(
            DEFAULT_PROMOTION_GATES, ticket.get('promotion_gates', {})
        ),
    }
    ...
    if strategy_overrides:
        if not isinstance(strategy_overrides, dict):
            raise ExportError(
                'ticket.strategy_overrides must be a mapping when provided'
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Build and validate without writing files',
    )
    parser.add_argument(
        '--generator-version',
        default='0.1.0',
