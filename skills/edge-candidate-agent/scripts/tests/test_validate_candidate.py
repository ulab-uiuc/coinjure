...
def test_validate_with_pipeline_uv_rejects_invalid_json(
    ...
    assert any(
        'failed to parse uv validation output as JSON' in error for error in errors
    )


def test_main_returns_error_for_missing_strategy(
    monkeypatch, capsys, tmp_path: Path
) -> None:
    missing_path = tmp_path / 'missing.yaml'
    monkeypatch.setattr(
        sys, 'argv', ['validate_candidate.py', '--strategy', str(missing_path)]
    )
...
def test_main_interface_only_success(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        sys, 'argv', ['validate_candidate.py', '--strategy', str(strategy_path)]
    )
    rc = validate_candidate.main()
    out = capsys.readouterr().out
    assert rc == 0
...
def test_main_strict_pipeline_requires_root(
    monkeypatch, capsys, tmp_path: Path
) -> None:
...
def test_main_pipeline_root_uses_schema_validator(
    monkeypatch, capsys, tmp_path: Path
) -> None:
