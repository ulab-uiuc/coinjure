...
def find_red_candle(
    weekly_candles: list[dict], earnings_week_idx: int = None
) -> Optional[dict]:
    """
    ...
    start_idx = 0
    end_idx = (
        earnings_week_idx if earnings_week_idx is not None else len(weekly_candles) - 1
    )
...
def analyze_weekly_pattern(
    ...
    for i, candle in enumerate(weekly_candles):
        if (
            candle['year'] == earnings_week_key[0]
            and candle['week'] == earnings_week_key[1]
        ):
            earnings_week_idx = i
            break
