...
def sample_iteration_history() -> dict:
    ...
    iterations = [
        make_iteration(i + 1, ev, f'Change {i + 1}') for i, ev in enumerate(evals)
    ]
...
def abandon_history() -> dict:
    ...
    return {
        ...
        'iterations': [
            make_iteration(i + 1, ev, f'Attempt {i + 1}') for i, ev in enumerate(evals)
        ],
    }
...
def healthy_history() -> dict:
    ...
    return {
        ...
        'iterations': [
            make_iteration(i + 1, ev, f'Improvement {i + 1}')
