...
ARCHETYPE_CATALOG = {
    ...
    'earnings_drift_pead': {
        ...
        'typical_failure_modes': [
            'One-day gap fills',
            'Market-wide selloffs overwhelming drift',
        ],
    },
    'volatility_contraction': {
        'hypothesis_type': 'breakout',
...
INVERSION_MAP = {
    'cost_defeat': [
        ...
        {
            'module': 'universe',
            'change': 'high_liquidity',
            'reason': 'Reduce slippage impact',
        },
    ],
    'tail_risk': [
        {
        ...
        {
            'module': 'structure',
            'change': 'market_neutral',
            'reason': 'Hedge directional risk',
        },
    ],
    'improvement_plateau': [
        {
        ...
        },
        {
            'module': 'entry',
            'change': 'timing_shift',
            'reason': 'Change entry timing mechanism',
        },
    ],
    'overfitting_proxy': [
        {
    ...
    for trigger_info in triggers_fired:
        ...
        for inv in inversions:
            ...
            for arch_id, arch in ARCHETYPE_CATALOG.items():
                ...
                proposal_id = sanitize_identifier(
                    f'pivot_{source_id}_inv_{trigger}_{arch_id}'
                )
...
def generate_objective_reframes(
    ...
    for trigger_info in triggers_fired:
        ...
        for arch_id in target_archetypes:
            ...
            proposal_id = sanitize_identifier(
                f'pivot_{source_id}_reframe_{trigger}_{arch_id}'
            )
            new_draft = _build_base_draft(draft, arch_id, arch, proposal_id)
            ...
            if reframe.get('new_criteria'):
                new_draft['validation_plan']['success_criteria'] = reframe[
                    'new_criteria'
                ]
...
def rank_and_select(
    ...
    for p in proposals:
        ...
        if targeted:
            quality = sum(
                score_quality_potential(t, target_arch) for t in targeted
            ) / len(targeted)
        else:
    ...
    if validation is not None:
        if not isinstance(validation, dict):
            ...
            if method != 'full_sample':
                errors.append(
                    "ticket.validation.method must be 'full_sample' for Phase I"
                )
            if validation.get('oos_ratio') is not None:
                errors.append(
                    'ticket.validation.oos_ratio must be omitted or null for Phase I'
                )
...
def write_outputs(
    ...
    for draft in selected:
        ...
        draft_path.write_text(
            yaml.safe_dump(draft_with_meta, sort_keys=False, allow_unicode=True)
        )
        ...
        if is_exportable:
            ...
            if ticket:
                ticket_filename = (
                    f"ticket_{draft['id'].replace('pivot_', '')}_{timestamp_str}.yaml"
                )
    ...
    manifest = {
        ...
        'exportable_count': sum(
            1 for d in manifest_drafts if d['category'] == 'exportable'
        ),
        'research_only_count': sum(
            1 for d in manifest_drafts if d['category'] == 'research_only'
        ),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--diagnosis', required=True, help='Path to pivot_diagnosis JSON'
    )
    parser.add_argument(
        '--strategy', required=True, help='Path to source strategy draft YAML'
    )
    parser.add_argument(
        '--max-pivots', type=int, default=3, help='Maximum pivot proposals'
    )
...
def main() -> int:
    ...
    all_proposals.extend(
        generate_inversions(source_draft, triggers_fired, source_archetype)
    )
    all_proposals.extend(
    ...
    for d in manifest['drafts']:
        print(
            f"  - {d['id']} ({d['category']}) combined={d['scores'].get('combined', 0):.2f}"
        )
