    ...
    for idx, it in enumerate(iterations):
        ...
        for dim_idx, dim in enumerate(ev['dimensions']):
            if not isinstance(dim, dict):
                raise ValueError(
                    f'iterations[{idx}].eval.dimensions[{dim_idx}] must be a mapping'
                )
            if 'name' not in dim:
                raise ValueError(
                    f"iterations[{idx}].eval.dimensions[{dim_idx}] missing 'name'"
                )
            if 'score' not in dim:
                raise ValueError(
                    f"iterations[{idx}].eval.dimensions[{dim_idx}] missing 'score'"
                )
...
def get_red_flag_ids(eval_data: dict[str, Any]) -> list[str]:
    ...
    return [
        f['id']
        for f in eval_data.get('red_flags', [])
...
def detect_overfitting_proxy(eval_data: dict[str, Any]) -> dict | None:
    ...
    if (
        ...
        return {
            ...
            'message': (
                'High in-sample scores with low robustness and curve-fitting red flags'
            ),
        }
    return None
...
def _determine_recommendation(
    ...
    if (
        iteration_count >= 3
        and score_trajectory[-1] < 30
        and len(score_trajectory) >= 3
    ):
...
def run_all_triggers(
    ...
    recommendation = _determine_recommendation(
        triggers_fired, score_trajectory, len(iterations)
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Detect stagnation in backtest iteration history.'
    )

    # Detection mode
    parser.add_argument(
        '--history', required=True, help='Path to iteration history JSON'
    )
...
def main() -> int:
    ...
    if args.append_eval:
        ...
        try:
            history = append_eval(
                eval_path, history_path, args.strategy_id, args.changes
            )
