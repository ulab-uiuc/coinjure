...
def _analyze_treasury_spread(treasury_rates: list[dict]) -> Optional[dict]:
    ...
    spread_series = sorted(
        spread_monthly.values(), key=lambda x: x['date'], reverse=True
    )
...
def _classify_curve_state(
    spread: float, sma_6m: Optional[float], roc_3m: Optional[float]
) -> str:
...
def _describe_signal(score: int, state: str, spread: float, direction: str) -> str:
    ...
    if score >= 60:
        ...
        return (
            f'SHIFTING: Yield curve {direction} ({state_label}, spread={spread:+.3f}%)'
        )
    else:
