...
class CorrelationAwareRiskManager(StandardRiskManager):
    """Risk manager that accounts for correlation between positions.

    Two 90%-correlated positions are effectively a concentrated bet.
    This manager adjusts position sizing by the portfolio correlation
    structure, preventing over-concentration in related markets.
    """
    ...
    def __init__(
        ...
        max_positions: int = 10,
        initial_capital: Decimal | None = None,
        max_correlated_exposure: Decimal = Decimal('10000'),
        ...
        # ticker_symbol -> ticker_symbol -> correlation
        self._correlations: dict[str, dict[str, float]] = {}

    def set_correlation(self, ticker_a: str, ticker_b: str, correlation: float) -> None:
        """Set the known correlation between two tickers."""
        self._correlations.setdefault(ticker_a, {})[ticker_b] = correlation
        self._correlations.setdefault(ticker_b, {})[ticker_a] = correlation
    ...
    def _check_correlated_exposure(
        ...
        ticker_corrs = self._correlations.get(ticker.symbol, {})
        if not ticker_corrs:
            return True
        ...
        for pos in self.position_manager.get_non_cash_positions():
            if pos.quantity <= 0:
                continue
            corr = ticker_corrs.get(pos.ticker.symbol, 0.0)
            if abs(corr) >= self.correlation_threshold:
                # Weight by correlation magnitude
                bid = self.market_data.get_best_bid(pos.ticker)
                pos_value = pos.quantity * (bid.price if bid else pos.average_cost)
                correlated_value += pos_value * Decimal(str(abs(corr)))

        total_correlated = correlated_value + new_trade_value
        if total_correlated > self.max_correlated_exposure:
            logger.warning(
                'Correlated exposure %.2f would exceed limit %.2f',
                total_correlated,
    ...
    async def check_trade(
        ...
        if not await super().check_trade(ticker, side, quantity, price):
            return False
        return self._check_correlated_exposure(ticker, side, quantity, price)
