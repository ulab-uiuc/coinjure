...
from coinjure.hub.hub import HUB_PID_PATH, HUB_SOCKET_PATH
...
_EXPLAIN_SYSTEM = """\
You are a prediction market trading analyst performing a deep validity review \
of a running spread/arbitrage strategy.

You will receive:
1. Strategy configuration (what markets it trades, what relationship it exploits).
2. Current performance data (PnL, positions, decision stats).
3. Market relation details (original hypothesis, spread type, reasoning).

Your job: assess whether this strategy is STILL VALID.

Consider:
- Has the original relationship changed? (rule updates, resolution criteria shifts)
- Is the spread still present and tradeable?
- Are there news events that could break the relationship?
- Is liquidity sufficient on both legs?
- Is execution quality acceptable? (slippage, partial fills, failed orders)
- Is the strategy actually profitable or losing money?

Return a JSON object:
{
  "verdict": "valid|degraded|invalidated",
  "confidence": 0.0-1.0,
  "validity_reasoning": "why the strategy is still valid or not",
  "risk_factors": ["list of current risks"],
  "recommendation": "hold|pause|reduce_size|retire",
  "recommendation_reasoning": "why this action",
  "estimated_remaining_edge": "high|medium|low|none"
}
"""
...
@click.option(
    ...
    'strategy_id',
    default=None,
    help='Analyze a single strategy (deep validity review). Omit to review all.',
)
@click.option('--model', default=None, help='LLM model (default: openai/gpt-4o).')
@click.option(
    '--execute/--no-execute',
    default=False,
    show_default=True,
    help='Actually execute recommended actions (all-strategies mode).',
)
@click.option('--json', 'as_json', is_flag=True, default=False, help='Emit JSON')
def engine_supervise(
    strategy_id: str | None,
    model: str | None,
    execute: bool,
    as_json: bool,
) -> None:
    """LLM supervisor: review strategies and recommend actions.

    With --id: deep validity analysis of a single strategy.
    Without --id: review all active strategies and recommend hold/pause/retire.
    """
    if strategy_id:
        _supervise_single(strategy_id, model, as_json)
    else:
        _supervise_all(model, execute, as_json)


def _supervise_single(
    strategy_id: str,
    model: str | None,
    as_json: bool,
) -> None:
    ...
    llm_model = model or os.environ.get('COINJURE_LLM_MODEL', 'openai/gpt-4o')
    reg = _load_registry()
    entry = reg.get(strategy_id)
    if entry is None:
        raise click.ClickException(f'Strategy not found: {strategy_id}')

    status_data: dict = {}
    sock = entry.socket_path
    if sock and Path(sock).exists():
        status_data = run_command('status', socket_path=Path(sock))
    ...
    ma = kwargs.get('market_a', {})
    mb = kwargs.get('market_b', {})
    if ma and mb:
        rid = f'{ma.get("id", "a")[:8]}-{mb.get("id", "b")[:8]}'
        rel = store.get(rid)
        if rel:
            relation_data = rel.to_dict()

    context_parts: list[str] = [
        f'Strategy ID: {entry.strategy_id}',
        f'Strategy ref: {entry.strategy_ref}',
        f'Lifecycle: {entry.lifecycle}',
        f'Created: {entry.created_at}',
        f'Exchange: {entry.exchange}',
        f'Paper PnL: {entry.paper_pnl or "unknown"}',
        f'Last signal: {entry.last_signal_at or "never"}',
    ]

    if entry.strategy_kwargs:
        context_parts.append(
            f'Strategy kwargs: {json.dumps(entry.strategy_kwargs, default=str)}'
        )

    if status_data.get('ok'):
        portfolio = status_data.get('portfolio', {})
        decision_stats = status_data.get('decision_stats', {})
        context_parts.append(f'\nLive status:')
        context_parts.append(f'  Total value: {portfolio.get("total", "?")}')
        context_parts.append(f'  Realized PnL: {portfolio.get("realized_pnl", "?")}')
        context_parts.append(f'  Event count: {status_data.get("event_count", 0)}')
        context_parts.append(f'  Orders: {status_data.get("orders", 0)}')
        context_parts.append(f'  Win rate: {decision_stats.get("win_rate", "?")}')
        context_parts.append(
            f'  Consecutive losses: {decision_stats.get("consecutive_losses", 0)}'
        )
        context_parts.append(f'  Runtime: {status_data.get("runtime", "?")}')
    else:
        context_parts.append('\nLive status: socket unreachable')

    if relation_data:
        context_parts.append(f'\nMarket relation:')
        context_parts.append(f'  Spread type: {relation_data.get("spread_type", "?")}')
        context_parts.append(f'  Hypothesis: {relation_data.get("hypothesis", "?")}')
        context_parts.append(
            f'  Original reasoning: {relation_data.get("reasoning", "?")}'
        )
        context_parts.append(f'  Confidence: {relation_data.get("confidence", "?")}')
        ma_info = relation_data.get('market_a', {})
        mb_info = relation_data.get('market_b', {})
        context_parts.append(
            f'  Market A: [{ma_info.get("platform", "?")}] {ma_info.get("question", "?")}'
        )
        context_parts.append(
            f'  Market B: [{mb_info.get("platform", "?")}] {mb_info.get("question", "?")}'
        )

    user_prompt = '\n'.join(context_parts)

    if not as_json:
        click.echo(f'Analyzing strategy {strategy_id} with {llm_model}...')
    ...
    click.echo('=' * 60)
    click.secho(f'  Verdict: {verdict}', fg=verdict_color, bold=True)
    click.echo(f'  Confidence: {explanation.get("confidence", "?")}')
    click.echo(f'  Edge remaining: {explanation.get("estimated_remaining_edge", "?")}')
    click.echo(f'  Recommendation: {explanation.get("recommendation", "?")}')
    click.echo(f'\n  Validity: {explanation.get("validity_reasoning", "")}')
    click.echo(f'  Recommendation: {explanation.get("recommendation_reasoning", "")}')
...
# ---------------------------------------------------------------------------
# Hub management (shared exchange data poller)
# ---------------------------------------------------------------------------
...
def engine_hub_status(socket_path: str, as_json: bool) -> None:
    ...
    if not socket.exists():
        ...
        return
    _emit(_send_hub_command(socket, 'status'), as_json=as_json)
...
def engine_hub_stop(socket_path: str, as_json: bool) -> None:
    ...
    if not socket.exists():
        ...
        return
    _emit(_send_hub_command(socket, 'stop'), as_json=as_json)
