...
class LivePolyMarketDataSource(DataSource):
            ...
            try:
                ...
                for event in events:
                    ...
                    if event_id not in self.processed_event_ids:
                        ...
                        is_cached = self._first_poll and event_id in self._cached_event_ids
                            ...
                            if not is_cached:
                                ...
                                for idx, token_id in enumerate(token_ids):
                                    ...
                                    if idx == 0 and len(token_ids) > 1:
                                        ...
                                        complement_id = token_ids[0]
                                    ticker = PolyMarketTicker(
                                        symbol=token_id, name=market_title,
                                        token_id=token_id, market_id=market_id,
                                        event_id=event_id, no_token_id=complement_id,
                                    )
                                    self._known_tickers[token_id] = ticker
    ...
    def _evict_stale_tickers(self) -> None:
        ...
        for tid, _ in evict_candidates[:num_to_evict]:
            ...
            stale_keys = [
                k for k in self.last_order_book_state
    ...
    async def _process_refresh_result(
        self, token_id: str, ticker: PolyMarketTicker, order_book: Any,
    ) -> None:
        ...
        for bid in order_book.bids:
            ...
            if size != prev_size:
                await self.event_queue.put(OrderBookEvent(
                    ticker=ticker, price=price, size=size,
                    size_delta=size - prev_size, side='bid',
                ))
        ...
        for ask in order_book.asks:
            ...
            if size != prev_size:
                await self.event_queue.put(OrderBookEvent(
                    ticker=ticker, price=price, size=size,
                    size_delta=size - prev_size, side='ask',
                ))
        ...
        for stale_key in old_bid_keys - seen_bid_keys:
            ...
            if prev_size > 0:
                await self.event_queue.put(OrderBookEvent(
                    ticker=ticker, price=Decimal(price_str),
                    size=Decimal('0'), size_delta=-prev_size, side='bid',
                ))
        for stale_key in old_ask_keys - seen_ask_keys:
            ...
            if prev_size > 0:
                await self.event_queue.put(OrderBookEvent(
                    ticker=ticker, price=Decimal(price_str),
                    size=Decimal('0'), size_delta=-prev_size, side='ask',
                ))
    ...
    async def _refresh_loop(self) -> None:
        ...
        while True:
            ...
            non_priority = [
                (tid, t) for tid, t in self._known_tickers.items()
                if tid not in self._priority_tokens
            ]
            if non_priority:
                batch_size = 20
            ...
            for result in results:
                ...
                try:
                    ...
                    logger.debug('Error processing refresh for %s', token_id, exc_info=True)
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if not isinstance(data, dict):
                            logger.warning('API returned non-dictionary response: %s', type(data))
                            return []

                        # Check if 'data' key exists
                        if 'data' not in data:
                            logger.warning(
                                "API response missing 'data' key. Available keys: %s", list(data.keys())
                            )
                            ...
                            for key, value in data.items():
                                if isinstance(value, list):
                                    logger.debug(
                                        "Found list under key '%s', trying this instead", key
                                    )
                                    return value
                        ...
                        if isinstance(articles_data, dict):
                            logger.debug("'data' is a dictionary, attempting to flatten")
                            flattened_articles = []

                            for category, articles in articles_data.items():
                                if isinstance(articles, list):
                                    logger.debug(
                                        "Found %d articles in category '%s'", len(articles), category
                                    )
                                    flattened_articles.extend(articles)
                                elif isinstance(articles, dict):
                                    logger.debug(
                                        "Category '%s' contains a dictionary, not a list", category
                                    )
                            ...
                            logger.debug(
                                'Flattened %d articles from categories', len(flattened_articles)
                            )
                            return flattened_articles

                        logger.warning("Unexpected 'data' format: %s", type(articles_data))
                        return []
    ...
    def _create_news_event(self, article: dict[str, Any]) -> NewsEvent:
        ...
        if 'published_at' in article and article['published_at']:
            try:
                ...
                logger.warning("Error parsing date '%s': %s", article.get('published_at'), e)
                ...
                for article in articles:
                    try:
                        ...
                        if not isinstance(article, dict):
                            logger.debug('Skipping non-dictionary article: %s', type(article))
                            continue
                        ...
                        if article_id in self.processed_article_ids:
                            logger.debug('Skipping already processed article: %s', article_id)
                            continue
                        ...
                        logger.debug(
                            'Added article to queue: %s - %s', article_id, article.get('title', 'No title')
                        )
