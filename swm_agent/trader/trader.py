...
class Trader(ABC):
    ...
    def _kill_switch_active(self) -> bool:
        ...
        Either:
        - SWM_KILL_SWITCH=1
        - SWM_KILL_SWITCH_FILE points to a file that exists
        """
        if os.environ.get('SWM_KILL_SWITCH', '').strip() == '1':
            return True
        kill_file = os.environ.get('SWM_KILL_SWITCH_FILE', '').strip()
        if kill_file:
            return os.path.exists(kill_file)
        default_kill_file = Path.home() / '.swm' / 'kill.switch'
